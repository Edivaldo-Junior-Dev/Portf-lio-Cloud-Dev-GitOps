      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-12">
           ...
           >
              ...
              {status === 'success' ? (
                <div className="text-center space-y-4">
                   ...
                   <a href={deployedUrl || '#'} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-xl font-bold transition-all transform hover:scale-105">
                     Abrir Site <ArrowRight size={16} />
                   </a>
                   <button onClick={() => { setStatus('idle'); setFile(null); setDeployedUrl(null); }} className="block w-full text-xs text-slate-500 hover:text-white mt-4 underline">Novo Deploy</button>
                </div>
              ) : (
                <>
                  ...
                  {file ? (
                    <div className="text-center space-y-4 w-full">
                       ...
                       <button onClick={handleDeploy} disabled={status === 'uploading'} className="w-full bg-orange-600 hover:bg-orange-500 text-white font-bold py-3 rounded-xl shadow-[0_0_20px_rgba(234,88,12,0.3)] transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <div className="bg-black/40 border border-slate-800 rounded-2xl p-6 font-mono text-xs flex flex-col h-[500px] shadow-2xl relative overflow-hidden">
           ...
           <div className="mt-6 flex-1 overflow-y-auto space-y-2 text-slate-300 p-2 scrollbar-thin scrollbar-thumb-slate-700">
              ...
              {logs.map((log, i) => (<div key={i} className="animate-fade-in-left border-l-2 border-slate-700 pl-2">{log}</div>))}
              {status === 'uploading' && (<div className="animate-pulse text-orange-400 mt-2">... Transferindo dados para nuvem ...</div>)}
           </div>
