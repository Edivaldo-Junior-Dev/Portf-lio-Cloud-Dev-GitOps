...
import { BookOpen, UserCheck, BarChart3, Sparkles, Settings, Code2, Database, Layers, CheckCircle2, ChevronDown, ChevronUp, Cpu, ShieldCheck, Cloud, Server, Globe, ArrowRight, LayoutList, Target, User, Flag, GitBranch, Infinity } from 'lucide-react';
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-20 animate-fade-in">
      ...
      <div className="space-y-4">
        ...
        {/* SECTION 1: PLANO DE BATALHA TRELLO */}
        ...
        {openSection === 'devops' && (
           <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 space-y-6 animate-fade-in-down">
             <div className="flex items-start gap-4">
                 ...
                 <div>
                    <h3 className="text-lg font-bold text-slate-800 dark:text-white">CI/CD Pipeline (AWS CodePipeline)</h3>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-1 leading-relaxed">
                        Atualmente, o deploy é manual (upload de .zip). O plano futuro visa implementar um pipeline de <strong>Integração e Entrega Contínua (CI/CD)</strong>, onde cada commit no GitHub dispara automaticamente o processo de build e deploy, eliminando intervenção humana.
                    </p>
                 </div>
             </div>

             <div className="bg-slate-50 dark:bg-slate-900 p-5 rounded-xl border border-slate-200 dark:border-slate-700">
                <h4 className="font-bold text-slate-800 dark:text-white mb-4 text-sm uppercase tracking-wide flex items-center gap-2">
                    <GitBranch size={16} className="text-purple-500" /> Arquitetura do Pipeline
                </h4>
                
                <div className="relative pl-8 space-y-6 before:absolute before:left-3.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200 dark:before:bg-slate-700">
                    {/* Step 1 */}
                    <div className="relative">
                        <div className="absolute -left-[29px] top-1 w-6 h-6 rounded-full bg-slate-900 text-white flex items-center justify-center text-[10px] font-bold border-4 border-white dark:border-slate-800">1</div>
                        <h5 className="font-bold text-slate-800 dark:text-white text-sm">Source (Gatilho)</h5>
                        <p className="text-xs text-slate-500 mt-1">
                            Desenvolvedor faz <code>git push</code> no GitHub. O AWS CodePipeline detecta a mudança via Webhook.
                        </p>
                    </div>

                    {/* Step 2 */}
                    <div className="relative">
                        <div className="absolute -left-[29px] top-1 w-6 h-6 rounded-full bg-slate-900 text-white flex items-center justify-center text-[10px] font-bold border-4 border-white dark:border-slate-800">2</div>
                        <h5 className="font-bold text-slate-800 dark:text-white text-sm">Build (Fábrica)</h5>
                        <p className="text-xs text-slate-500 mt-1">
                            <strong>AWS CodeBuild</strong> inicia um container, instala dependências (<code>npm install</code>), roda testes automatizados e compila o projeto (<code>npm run build</code>).
                        </p>
                    </div>

                    {/* Step 3 */}
                    <div className="relative">
                        <div className="absolute -left-[29px] top-1 w-6 h-6 rounded-full bg-slate-900 text-white flex items-center justify-center text-[10px] font-bold border-4 border-white dark:border-slate-800">3</div>
                        <h5 className="font-bold text-slate-800 dark:text-white text-sm">Deploy (Entrega)</h5>
                        <p className="text-xs text-slate-500 mt-1">
                            O artefato gerado é enviado para o Bucket S3 de produção (Frontend) ou atualiza as funções Lambda via CloudFormation (Backend).
                        </p>
                    </div>
                </div>
             </div>
